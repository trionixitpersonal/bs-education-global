...
import { DocumentationDialog } from "@/components/admin/documentation/DocumentationDialog";
...
export default function DocumentationAdminPage() {
  ...
  const handleEdit = (guide: DocumentGuide) => {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Are you sure you want to delete this documentation guide?")) return;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold text-gray-900">Documentation Support Management</h1>
              ...
              {guides.length === 0 ? (
                ...
                guides.map((guide) => (
                  <tr key={guide.id} className="hover:bg-gray-50">
                    ...
                    <td className="whitespace-nowrap px-6 py-4 text-gray-500">
                      {guide.documents?.length || 0} items
                    </td>
      ...
      {isDialogOpen && <DocumentationDialog guide={editingGuide} onClose={handleDialogClose} />}
    </div>
  );
}
